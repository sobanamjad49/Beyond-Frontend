...
import { useProducts } from "./ProductContext"; // ✅ use context

function SummerPret() {
  const { products, loading } = useProducts(); // ✅ global product state
  const [columns, setColumns] = useState(2);
  const [summerPretItems, setSummerPretItems] = useState([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    const filtered = products.filter(item => item.category === "SummerPret56");
    setSummerPretItems(filtered);
  }, [products]);
  ...
  return (
    <div className="p-4">
      ...
      <div className={`grid ${gridClass} gap-4`}>
        {summerPretItems.map((item, index) => (
