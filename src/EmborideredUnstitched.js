...
function EmborideredUnstitched() {
  const { products, loading } = useProducts();
  const [columns, setColumns] = useState(2);
  const [embUnstitchedItems, setEmbUnstitchedItems] = useState([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    const filtered = products.filter(
      (item) => item.category === "EmborideredUnstitchedimages"
    );
    setEmbUnstitchedItems(filtered);
  }, [products]);
  ...
  return (
    <div className="p-4">
      ...
      <div className={`grid ${gridClass} gap-4`}>
        {embUnstitchedItems.map((item, index) => (
...
export default EmborideredUnstitched;
