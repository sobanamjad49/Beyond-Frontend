...
import { useProducts } from "./ProductContext"; // ✅ using shared context

function FestivePret() {
  const { products, loading } = useProducts();
  const [columns, setColumns] = useState(2);
  const [festivePretItems, setFestivePretItems] = useState([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    const filtered = products.filter(
      (item) => item.category === "FestivePretimages"
    );
    setFestivePretItems(filtered);
  }, [products]);
  ...
  return (
    <div className="p-4">
      <h2 className="text-center text-4xl font-medium mb-4">
      ...
      <div className={`grid ${gridClass} gap-4`}>
        {festivePretItems.map((item, index) => (
