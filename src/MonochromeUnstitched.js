...
function MonochromeUnstitched() {
  const { products, loading } = useProducts(); // ✅ use shared data context
  const [columns, setColumns] = useState(2);
  const [monochromeUnstitchedItems, setMonochromeUnstitchedItems] = useState([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    const filtered = products.filter(
      (item) => item.category === "MonochromeUnstitchedimages"
    );
    setMonochromeUnstitchedItems(filtered);
  }, [products]);
  ...
  return (
    <div className="p-4">
      ...
      <div className={`grid ${gridClass} gap-4`}>
        {monochromeUnstitchedItems.map((item, index) => (
...
export default MonochromeUnstitched;
