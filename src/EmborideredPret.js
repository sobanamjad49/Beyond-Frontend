...
function EmborideredPret() {
  const { products, loading } = useProducts();
  const [columns, setColumns] = useState(2);
  const [embPretItems, setEmbPretItems] = useState([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    const filtered = products.filter(
      (item) => item.category === "EmborideredPretimages"
    );
    setEmbPretItems(filtered);
  }, [products]);
...
export default EmborideredPret;
